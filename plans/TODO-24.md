...
to stand for the two feedback items already in the backlog. TODO-34 (a skill that reviews all the work
done in a session) and TODO-35 (run it when out of work) both need *something* that has seen more than
...
5. **`docs/ARCHITECTURE.md`** — subsection under *Continuous operation and scaling*: the dispatcher
   ...
   dependency: TODO-34's feedback skill reads the dispatcher's reports.
...
- **The dispatcher accumulates reports.** Smaller than diffs, but not nothing over fifty iterations.
  Bounding the report to three fields is the mitigation; if it still grows, the fix belongs to TODO-34
  (which consumes them), not here.
- **Interaction with TODO-31** (capture lessons in `CLAUDE.md`). That item exists *because* iteration
  context is lost, and this change makes the loss structural rather than incidental. It strengthens the
  case for TODO-31 rather than conflicting with it — worth one sentence in the ARCHITECTURE subsection
